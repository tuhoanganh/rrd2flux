...
do
  ...
  export COUNT=$(rrdtool fetch $DIR/$i/Juniper_Temperature_ACX.rrd  MAX -e $LAST_TIME -s $FIRST_TIME | grep -v 'nan'| sed -e 's/://g'| sed '1d' |grep -Ev '^$' | wc -l)
  if [[ $COUNT == 0 ]]; then
    echo $i "has not entry!"
  else
    echo $i
    export BEGIN_DATA_TIME=$(rrdtool fetch $DIR/$i/Juniper_Temperature_ACX.rrd  MAX -e $LAST_TIME -s $FIRST_TIME | grep -v 'nan'| sed -e 's/://g'| sed '1d' |grep -Ev '^$' | head -n 1| awk '{print $1}')
    ./acx/rrdflux_temperature_acx.py -f $DIR/$i/Juniper_Temperature_ACX.rrd -D $HOSTNAME -H $HOST -p $PORT -d $DB -U $USER -P $PASSWORD -F $BEGIN_DATA_TIME -L $LAST_TIME -R $RRD_MIN_RES
  fi
done
