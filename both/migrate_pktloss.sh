...
do
  ...
  echo $i
  for j in `find $DIR/$i/pktLoss*.rrd -printf "%f\n"`
  do
    ...
    if [[ $COUNT == 0 ]]; then
      ...
      ./both/rrdflux_pktloss.py -f$DIR/$i/$j -D $HOSTNAME -H $HOST -p $PORT -d $DB -U $USER -P $PASSWORD --service $j -F $BEGIN_DATA_TIME -L $LAST_TIME -R $RRD_MIN_RES
    fi
  done
done
